...
We ask respondents  some standard sociodemographics. But we also include an item from the [GLES Panel](https://doi.org/10.4232/1.14114) on energy transformation: "From 2030, no more new cars with petrol or diesel engines are to be registered in Germany. Do you agree?" (entrans). Since we cannot share the real data we created some fake data using the [`faux` package](https://cran.r-project.org/web/packages/faux/index.html).
...
On the district level, we have already most of our information and/or created the indicator yesterday.
...
head(sampling_area_attributes, 4)
...
*Access to charging infrastructure*
> Charging stations per 1000 inhabitant in a district
...
We have our nice fake coordinates and we do know that we also have variation within some districts (E.g. cologne), concerning e-car mobility.
So let's try to operationalize the variables on a smaller level of aggregation.

*Access to charging infrastructure*
> Charging stations in a 2000m buffer

*Alternative means of transport*
> Distance to closest train stop

*Rural-urban divide*
> Population in a 2000m buffer 

---

## Charging stations in 2000m buffer

To calculate the number of chargers in a 2km buffer, the procedure is very similar to calculating the buffers in a district.

```{r charging-stations}

# Create 2000m buffers around the fake coordinates
buffers <- 
  fake_coordinates %>%
...
To measure access to alternative transportation (e.g. public transport), we want to measure the distance to the closest train station for each respondent. 
...
# takes a while, so sneaky preperation
nrw_pt_trainstops <- sf::st_read("./data/nrw_pt_osmtrainstops.shp", crs = 3035)
...
`sf::st_distance()` will calculate between **all** respondents and **all** train stations resulting in a matrix with 1,786,000 objects (1,000 respondent * 1,786 hospitals). We can make our lives a little bit easier by first identifying the nearest station and then calculate the distance.
...
...and we're not yet done: we still need the population in the neighborhood. Let's calculate buffers of 2000 meters and add the population mean values to our dataset.
