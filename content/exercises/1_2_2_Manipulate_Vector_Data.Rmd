...
Calculate the number of charger per district (`charger_count`) and the number of charger per 1,000 inhabitant in each district (`charger_dens`). 
...
```{r second-exercise, solution = TRUE, cache = TRUE}
...
cologne_sur_enhanced  <-
  ...
  sf::st_join(., cologne_sur, join = st_within) %>% 
